...
public class MetaQuest3Controller : MonoBehaviour
{
    ...
    private Service<string, string> toggleOcclusionService;

    void Start()
    {
        _publisher = new Publisher<MetaQuest3InputData>("InputData", false);
        IRXRNetManager _netManager = IRXRNetManager.Instance;
        toggleOcclusionService = new Service<string, string>("ToggleOcclusion", ToggleOcclusion);
    ...
    private string ToggleOcclusion(string data)
    {
        Debug.Log("Toggle Occlusion");
        if (environmentDepthManager == null)
        {
            return IRXRSignal.ERROR;
        }
        if (environmentDepthManager.OcclusionShadersMode == OcclusionShadersMode.None)
        {
