...
public class SceneLoader : MonoBehaviour {
  ...
  GameObject CreateObject(Transform root, SimBody body) {
    ...
    if (_simObjTrans.ContainsKey(body.name)) Debug.LogWarning($"Duplicate object name found: {body.name}");
    _simObjTrans[body.name] = bodyRoot.transform;
    return bodyRoot;
  }
...
public T[] DecodeArray<T>(byte[] data, int start, int length) where T : struct
{
